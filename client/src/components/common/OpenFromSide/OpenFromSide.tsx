...
const OpenFromSide: React.FC<OpenFromSideProps> = ({
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    ...
    const edgeThreshold = 30; // 30px from the left edge of the screen
    
    // Only start dragging if touch begins near the left edge
  ...
  const panelStyle = {
    ...
    transition: isDragging
      ? "none"
      : "transform 0.3s ease-in-out, right 0.4s ease-in-out",
  };
